...
- **Match previews and identifiers**: the first few recent matches (`PREVIEW_COUNT` in `MatchList.tsx`, currently 3) get an *immediate* rich preview (map/mode/placement/kills/damage), rendered as an explicit equal-width grid (responsive `1fr` → `repeat(2,1fr)` → `repeat(3,1fr)`), not `auto-fill` (which leaves dead space instead of spanning the full row). This count is deliberately capped low to stay within PUBG's 10 req/min free-tier rate limit alongside the other calls a single search already makes (player + season stats) — don't raise it without checking `docs/ARCHITECTURE.md`'s known-limitations section first. Matches beyond that count are **never** labeled "Match N" (a raw index is not a meaningful identifier to a user), but they **do** all load automatically in the end — real per-match info (date/map/placement) costs one PUBG call each, so a background queue (see the constants + effect above `MatchList`'s `runQueue`) fetches them one at a time, paced ~6.5s apart, instead of firing all at once. A click on any row still works and jumps that one match ahead of the queue immediately (`handleSelect`). If a background fetch gets a 429, it backs off (using `Retry-After` if present, a fixed default otherwise) and retries a few times before giving up on that one match — the queue itself never gets abandoned by a single rate-limited item. **This was a deliberate reversal of an earlier "never auto-load the rest" rule** — the user explicitly asked for the older matches to appear without tapping, so the trade-off was re-made consciously (paced background load instead of instant-but-rate-limit-breaking, or forever-manual). Don't revert to require-a-click without checking whether that's still wanted; don't remove the pacing/backoff to "make it faster" without re-checking the rate limit constraint first.
