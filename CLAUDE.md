...
**Known, deliberately-not-done**: "vs. your average" style comparisons on match stat tiles would need a new backend metric (per-stat season averages) that doesn't exist yet (`SeasonStatsDto` only has wins/roundsPlayed/winRate) — flagged, not implemented, since it's a backend+frontend change and this pass was scoped frontend-only. **Update**: this one is now done — `SeasonStatsDto` carries the full stat set + a 6-axis `radar`/`archetype`, `PerformanceRadar.tsx`/`PlayerIdentityCard.tsx`/`PerformanceBreakdown.tsx` render it, and `MatchList.tsx`'s selected-match panel shows match-vs-season-average delta chips.

---

# Not Yet Built (identified gaps vs. the V2 vision in the backend's `CLAUDE.md`)

Three things still missing, roughly in order of value-for-effort:

1. **Season trend charts (real time series, not just one delta).** Right now "Season Performance" only compares the current season to the immediately previous one (`SeasonComparison` — a single before/after delta per metric). The V2 vision wants an actual trend *line* (win rate, avg damage, K/D, headshot rate over time). PUBG's API has no native "stats over the last N weeks" endpoint — the only real data points available are whole seasons, and a season changes every 2-3 months, so there are at most a handful of historical points, not a smooth line. Realistic version: a small multi-point line/sparkline chart plotting each of the last few seasons (not just current-vs-previous), once the backend fetches more than one prior season. **This needs a backend change first** (currently only fetches the ONE previous season, see `PubgApiClient.findPreviousSeasonId()`/`PlayerService.getSeasonStats()` in the backend repo) — don't build the frontend chart against fake multi-season data.

2. **Deep Insights** (favorite map, best/worst performing map, longest survival streak, most aggressive match, most efficient win, etc.). Feasible in principle — `MatchDto` already has `mapName`/`gameMode`/per-match stats — but computing any of these needs stats aggregated *across every recent match*, not just the currently-selected one. That means either the backend does the aggregation (preferred — one endpoint that scans the player's cached/fetched matches and returns the derived insights) or the frontend would have to fetch every match individually, which is expensive against PUBG's shared 10 req/min rate limit (see the `MatchList.tsx` rate-limit notes above). **This needs a new backend endpoint** — don't build a frontend version that fetches N matches in a loop just to compute one "favorite map" badge.

3. **Badges / achievements / progress rings.** Purely a frontend presentation layer — no new data needed beyond what already exists (archetype, radar scores, delta chips, season stats). This one genuinely *can* be built frontend-only: e.g. a small badge row under `PlayerIdentityCard` for milestone-style facts already computable from `SeasonStats`/`MatchDto` (e.g. "100+ rounds this season", "Top 10 rate above 40%", "Personal best damage this match" using the existing match-vs-season delta). Lowest effort, purely visual — a good candidate to build first if picking up this list, since it doesn't block on any backend work.
